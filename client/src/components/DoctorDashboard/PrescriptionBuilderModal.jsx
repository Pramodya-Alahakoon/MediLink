...
import { X, Plus, Trash2, Pill, Activity, Calendar, FileText, Loader2, Save } from 'lucide-react';
...
const PrescriptionBuilderModal = ({ isOpen, onClose, appointment, doctorId, onSuccess }) => {
    ...
    try {
      ...
      if (data.success || data.data) {
        ...
        if (appointment.status === 'Confirmed' || appointment.status === 'Pending') {
          try {
             await customFetch.put(`/api/doctors/appointments/${appointment._id}/complete`);
          } catch(e) { /* ignore auto-completion error if endpoint is missing */ }
        }
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[20px] font-bold text-[#0D1C2E]">Issue E-Prescription</h2>
              <p className="text-[13px] font-medium text-slate-500">
                Patient: <span className="font-bold text-slate-700">{appointment.patientName || 'Unknown'}</span>
              </p>
            </div>
          </div>
          <button 
        ...
        <div className="flex-1 overflow-y-auto w-full custom-scrollbar">
          <form id="prescription-form" onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
            
            {/* Context / Diagnosis Section */}
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4 text-[#055153]">
                <Activity size={18} />
                <h3 className="font-bold text-[15px]">Clinical Diagnosis & Notes</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="md:col-span-2">
                  <label className="block text-[13px] font-bold text-slate-700 mb-1.5">Primary Diagnosis <span className="text-rose-500">*</span></label>
                  <input 
                    required 
                    maxLength={500}
                    value={diagnosis}
                ...
                <div className="md:col-span-2">
                  <label className="block text-[13px] font-bold text-slate-700 mb-1.5">Physician Notes (Optional)</label>
                  <textarea 
                    maxLength={1000}
                    value={notes}
                ...
                <div className="md:col-span-1">
                  <label className="block text-[13px] font-bold text-slate-700 mb-1.5">Follow-up Date (Optional)</label>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-[#055153]">
                  <Pill size={18} />
                  <h3 className="font-bold text-[15px]">Prescribed Medicines</h3>
                </div>
                <button 
                  type="button" 
              ...
              <div className="space-y-4">
                {medicines.map((med, index) => (
                  <div key={index} className="relative bg-[#F8FAFB] rounded-2xl p-5 border border-slate-100 group transition-all hover:border-slate-200 hover:bg-white hover:shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                      ...
                      <div className="md:col-span-4">
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Medicine Name</label>
                        <input
                      ...
                      <div className="md:col-span-3">
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Dosage</label>
                        <input
                      ...
                      {/* Frequency - 3 columns */}
                      <div className="md:col-span-3">
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Frequency</label>
                        <select
                          value={med.frequency}
                      ...
                      {/* Duration - 2 columns */}
                      <div className="md:col-span-2">
                        <label className="block text-[12px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Duration</label>
                        <input
