  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        ...
        const errorMsg = err?.response?.data?.msg || err?.response?.data?.message || 'Failed to load appointments';
        setError(errorMsg);
    ...
    <div className="min-h-screen bg-[#FAFAFB] dark:bg-slate-900 pb-20">
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-6 py-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-[#112429] dark:text-white">My Appointments</h1>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-[#4B5A69] dark:text-slate-400 mt-3">
            Showing {filteredAppointments.length} {filterStatus !== 'All' ? filterStatus.toLowerCase() : ''} appointment{filteredAppointments.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[#4B5A69] dark:text-slate-400 font-medium">Loading your appointments...</p>
          </div>
        ) : error ? (
              ...
              >
                <div className="p-6">
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6 pb-6 border-b border-gray-100 dark:border-slate-700">
                    ...
                    <span className={`px-4 py-2 rounded-full font-bold text-sm whitespace-nowrap ${getStatusBadge(appointment.status)}`}>
                  ...
                  {appointment.preMedicationSteps && appointment.preMedicationSteps.length > 0 && (
                    <div className="mb-6 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
                      ...
                      <ul className="space-y-2">
                        {appointment.preMedicationSteps.map((step, idx) => (
                          <li key={idx} className="flex items-start gap-3 text-sm text-emerald-800 dark:text-emerald-300">
