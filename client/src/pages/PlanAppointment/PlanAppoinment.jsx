...
function PlanAppointment() {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        const uniqueSpecialties = [...new Set(docs.map(d => d.specialization).filter(Boolean))];
  ...
  const fetchAvailability = async (doctor) => {
    ...
    try {
      ...
      if (res.data.success) {
         ...
         const firstAvailable = days.find(d => !d.isBlocked && d.isWorkingDay) || days[0];
  ...
  const handleFinalBooking = async () => {
    ...
    try {
      ...
      const { data: aptData } = await customFetch.post('/api/appointments', appointmentPayload);
      ...
      try {
        await customFetch.post(`/api/availability/slots/${selectedSlot._id}/book`, {
          ...
          patientName,
          appointmentType: consultationType === 'Video' ? 'Video Consultation' : 'In-Person Visit',
        });
      } catch (slotErr) {
        // Non-fatal: appointment already created; log and continue
        console.warn('Slot booking warning:', slotErr?.response?.data?.message || slotErr.message);
      }
      ...
      try {
        ...
        console.warn('Payment service unavailable:', paymentErr?.response?.data?.message || paymentErr.message);
      }
      ...
      const msg = err?.response?.data?.msg || err?.response?.data?.message || 'Booking failed. Please try again.';
      toast.error(msg);
    } finally {
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="flex items-center justify-between relative">
            ...
            {steps.map((step) => {
              ...
              const isPast = currentStep > step.id && !(currentStep === 4 && step.id === 3);
              return (
                <div key={step.id} className="flex flex-col items-center bg-[#FAFAFB] dark:bg-slate-900 px-4">
                  <div 
                  ...
                  <span className={`text-[11px] font-bold tracking-wide ${
                    isActive ? 'text-[#055153] dark:text-teal-400' : 'text-gray-400 dark:text-gray-500'
                  }`}>
                    {step.label}
                  </span>
                </div>
        ...
        <>
            ...
            >
              <div className="mb-10">
                <h1 className="text-4xl md:text-[44px] font-extrabold text-[#112429] dark:text-white tracking-tight mb-4">
                  Select your <span className="text-[#055153] dark:text-teal-400 relative">
                    specialist.
                    <svg className="absolute w-full h-3 -bottom-1 left-0 text-teal-200/50" viewBox="0 0 100 20" preserveAspectRatio="none">
                ...
                <p className="text-[#4B5A69] dark:text-slate-400 text-lg max-w-2xl leading-relaxed">
                  Connect with world-class medical professionals verified by our clinical editorial board. Precision care, human empathy.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
                {isLoadingDoctors ? (
                   Array.from({ length: 3 }).map((_, i) => (
                      <div key={i} className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 animate-pulse h-[320px]"></div>
                   ))
                    ...
                    >
                      <div className="flex items-start gap-4 mb-6 relative">
                        <div className="relative">
                          <img 
                            src={doc.profileImage?.includes('http') ? doc.profileImage : 'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?q=80&w=256&auto=format&fit=crop'} 
                            alt={doc.name}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <span className="text-xs text-gray-400">({doc.rating?.count || '0'} reviews)</span>
                          </div>
                      ...
                      <div className="space-y-4 mb-7 flex-1 border-t border-gray-100 pt-5">
                        <div className="flex items-center justify-between text-[13px]">
                          <div className="flex items-center gap-2.5 text-[#4B5A69] font-medium">
                            <FiCalendar className="w-4 h-4 text-[#055153]" /> Next Slot
                          </div>
                          <div className="font-bold text-[#112429]">Tomorrow, 09:30 AM</div>
                        </div>
                        <div className="flex items-center justify-between text-[13px]">
                          <div className="flex items-center gap-2.5 text-[#4B5A69] font-medium">
                            <FiCreditCard className="w-4 h-4 text-[#055153]" /> Consultation
                          </div>
                          <div className="font-bold text-[#112429]">Rs. 2500.00</div>
                        </div>
                      </div>
              ...
              {/* Why Book With Us */}
              <div className="mb-20">
                <h2 className="text-2xl font-extrabold text-[#112429] dark:text-white mb-8">Why book with us?</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="rounded-[28px] p-8 bg-[#F8FAFC] border border-[#EEF2F6]">
                    ...
                    <h3 className="text-lg font-bold text-[#112429] mb-3">Verified Experts</h3>
                    <p className="text-[#64748B] text-[14px] leading-relaxed font-medium">
                      Every clinician is rigorously vetted by our editorial board for credentials and experience.
                    </p>
                  </div>
                  {/* Card 2 */}
                  <div className="rounded-[28px] p-8 bg-[#055153] border border-[#044143] text-white shadow-xl shadow-[#055153]/20">
                    ...
                    <h3 className="text-lg font-bold text-white mb-3">Instant Booking</h3>
                    <p className="text-white/80 text-[14px] leading-relaxed font-medium">
                      No phone calls, no waiting. Secure your appointment in under 60 seconds with real-time sync.
                    </p>
                  </div>
                  {/* Card 3 */}
                  <div className="rounded-[28px] p-8 bg-[#F8FAFC] border border-[#EEF2F6]">
                    ...
                    <h3 className="text-lg font-bold text-[#112429] mb-3">Data Privacy</h3>
                    <p className="text-[#64748B] text-[14px] leading-relaxed font-medium">
                      Your medical records and personal details are encrypted using clinical-grade security protocols.
                    </p>
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <div>
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
                  <div>
                    <h2 className="text-3xl font-extrabold text-[#112429] dark:text-white mb-2">Available Slots</h2>
                    <p className="text-[#4B5A69] dark:text-slate-400">Select a time that works best for your schedule.</p>
                  </div>
                  ...
                  <div className="bg-[#EEF5F9] dark:bg-slate-800 text-[#055153] dark:text-teal-400 px-4 py-2.5 rounded-full font-bold text-sm flex items-center justify-center gap-2 border border-[#E1EEF4] dark:border-slate-700 w-fit">
                    ...
                    {weekRange ? formatMonthRange(weekRange.start, weekRange.end) : 'Loading...'}
                  </div>
                </div>
                    ...
                    <>
                      <div className="flex justify-between items-center overflow-x-auto gap-2 pb-2 mb-6 scrollbar-hide">
                        {availabilityData.map((dayData, i) => {
                          const isSelected = selectedDateObj?.date === dayData.date;
                          const isDisabled = dayData.isBlocked || !dayData.isWorkingDay;
                          const dayShort = dayData.day.substring(0, 3).toUpperCase();
                          
                          return (
                            <button 
                              key={i}
                              disabled={isDisabled}
                              onClick={() => { setSelectedDateObj(dayData); setSelectedSlot(null); }}
                            ...
                            >
                              <span className="text-[10px] tracking-wider mb-1">{dayShort}</span>
                              <span className="text-xl">{dayData.dayNumber}</span>
                            </button>
                          )
                        })}
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-b from-[#F2FDFE] to-white dark:from-slate-800 dark:to-slate-800 rounded-[32px] p-8 border border-[#E0F3F4] dark:border-slate-700 shadow-sm relative overflow-hidden">
                   ...
                   <div className="flex flex-col flex-col items-center text-center relative z-10 mb-8 pt-4">
                      <div className="relative mb-5">
                         <img 
                           src={selectedDoctor?.profileImage?.includes('http') ? selectedDoctor.profileImage : 'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?q=80&w=256&auto=format&fit=crop'} 
                   ...
                   <div className="space-y-6 pt-6 border-t border-gray-100 dark:border-slate-700">
                      <div className="flex gap-4 items-start">
                         ...
                         <div>
                           <p className="text-[10px] font-bold tracking-widest text-[#64748B] uppercase mb-0.5">Location</p>
                           <p className="font-bold text-[#112429] dark:text-white text-[13px]">{selectedDoctor?.location || 'Central Medical Plaza, Suite 402'}</p>
                         </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start">
                         ...
                         <div>
                           <p className="text-[10px] font-bold tracking-widest text-[#64748B] uppercase mb-0.5">Fee</p>
                           <p className="font-bold text-[#112429] dark:text-white text-[13px]">Rs. 2500.00 per session</p>
                         </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start">
                         ...
                         <div>
                           <p className="text-[10px] font-bold tracking-widest text-[#64748B] uppercase mb-0.5">Next Step</p>
                           <p className="text-[#112429] dark:text-white text-[13px] italic font-semibold">Insurance Verification</p>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="mt-8 bg-[#EFFFFF] dark:bg-teal-900/20 border border-[#BAEBEB] dark:border-teal-800 rounded-2xl p-5">
                      <div className="text-[10px] font-extrabold tracking-widest text-[#055153] dark:text-teal-500 uppercase mb-3">Booking Summary</div>
                      <div className="flex justify-between items-center gap-4">
                        <span className="text-[13px] text-[#4B5A69] dark:text-slate-400 font-medium">Selected Time</span>
                        <span className="text-[13px] font-bold text-[#112429] dark:text-white text-right">
                          {selectedSlot ? `${formatShortDate(selectedDateObj?.date)}, ${selectedSlot.startTime}` : '---'}
                        </span>
                      </div>
                   </div>
                </div>
                ...
                <div className="bg-[#1C2C3E] rounded-[32px] p-8 text-white relative overflow-hidden">
                   ...
                   <h3 className="font-bold text-lg mb-3 relative z-10">Need assistance?</h3>
                   <p className="text-[#9BAEC1] text-[13px] leading-relaxed mb-6 font-medium relative z-10 pr-4">
                     Our patient coordinators are available 24/7 to help with your booking.
                   </p>
          ...
          {currentStep === 3 && (
            ...
            >
              ...
              <div>
                <div className="mb-8">
                  <h2 className="text-3xl font-extrabold text-[#112429] dark:text-white mb-2">Patient Details</h2>
                  <p className="text-[#4B5A69] dark:text-slate-400">Please provide your details and symptoms to finalize your booking.</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-[28px] p-8 shadow-sm border border-gray-100 dark:border-slate-700 mb-8 space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-[#112429] dark:text-gray-200 mb-3">Consultation Category</label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                       ...
                       >
                         ...
                         <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${consultationType === 'In-Person' ? 'bg-[#055153] text-white' : 'bg-[#EEF5F9] dark:bg-slate-700 text-[#055153] dark:text-teal-400'}`}>
                            <FiMapPin className="w-5 h-5" />
                         </div>
                         <h4 className={`font-bold text-[15px] mb-1 ${consultationType === 'In-Person' ? 'text-[#055153] dark:text-teal-300' : 'text-[#112429] dark:text-white'}`}>In-Person Visit</h4>
                         <p className="text-xs text-[#64748B] font-medium leading-relaxed">Visit the doctor physically at the clinical location.</p>
                       </button>
                       ...
                       >
                         ...
                         <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${consultationType === 'Video' ? 'bg-[#055153] text-white' : 'bg-[#EEF5F9] dark:bg-slate-700 text-[#055153] dark:text-teal-400'}`}>
                         ...
                         <h4 className={`font-bold text-[15px] mb-1 ${consultationType === 'Video' ? 'text-[#055153] dark:text-teal-300' : 'text-[#112429] dark:text-white'}`}>Video Consultation</h4>
                         <p className="text-xs text-[#64748B] font-medium leading-relaxed">Secure online telemedicine video room connection.</p>
                       </button>
                    </div>
                  </div>
                  ...
                  {/* Full Name */}
                  <div>
                    <label className="block text-sm font-bold text-[#112429] dark:text-gray-200 mb-2">Full Name</label>
                    <input 
                      type="text" 
                      value={patientName}
                  ...
                  {/* Phone Number */}
                  <div>
                    <label className="block text-sm font-bold text-[#112429] dark:text-gray-200 mb-2">Contact Phone</label>
                    <input 
                      type="tel" 
                      value={contactPhone}
                    ...
                    <p className="text-xs text-slate-500 mt-2 flex items-center gap-1.5 font-medium">
                      <FiShield className="text-[#055153] w-3.5 h-3.5" /> We strictly protect your privacy.
                    </p>
                  </div>
                ...
                <div className="flex justify-between items-center pt-4">
                  ...
                  <button 
                    disabled={!patientName || contactPhone.length !== 10 || !symptoms}
                    onClick={() => {
                        if (contactPhone.length !== 10) { toast.error("Please enter a valid 10-digit phone number"); return; }
                        setCurrentStep(4);
                    }}
                    className={`px-8 py-3.5 rounded-xl font-bold transition-all duration-300 shadow-xl ${
                ...
                <div className="bg-gradient-to-b from-[#F2FDFE] to-white dark:from-slate-800 dark:to-slate-800 rounded-[32px] p-8 border border-[#E0F3F4] dark:border-slate-700 shadow-sm relative overflow-hidden">
                   ...
                   <div className="space-y-6 pt-6 border-t border-gray-100 dark:border-slate-700">
                      <div className="flex gap-4 items-start">
                         ...
                         <div>
                           ...
                           <p className="font-bold text-[#112429] dark:text-white text-[13px]">
                             {consultationType === 'In-Person' ? (selectedDoctor?.location || 'Central Medical Plaza, Suite 402') : 'Virtual Telemedicine Room'}
                           </p>
                         </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start">
                         ...
                         <div>
                           <p className="text-[10px] font-bold tracking-widest text-[#64748B] uppercase mb-0.5">Next Step</p>
                           <p className="text-[#112429] dark:text-white text-[13px] italic font-semibold">Final Review</p>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="mt-8 bg-[#EFFFFF] dark:bg-teal-900/20 border border-[#BAEBEB] dark:border-teal-800 rounded-2xl p-5">
                      <div className="text-[10px] font-extrabold tracking-widest text-[#055153] dark:text-teal-500 uppercase mb-3">Booking Summary</div>
                      <div className="flex justify-between items-center gap-4">
                        <span className="text-[13px] text-[#4B5A69] dark:bg-slate-400 font-medium">Selected Time</span>
                        <span className="text-[13px] font-bold text-[#112429] dark:text-white text-right">
                          {selectedSlot ? `${formatShortDate(selectedDateObj?.date)}, ${selectedSlot.startTime}` : '---'}
                        </span>
                      </div>
          ...
          {bookingSuccess ? (
            ...
            <div className="fixed inset-0 bg-[#FAFAFB] dark:bg-slate-900 z-50 flex items-center justify-center px-6">
              ...
              >
                ...
                <h1 className="text-4xl font-extrabold text-[#112429] dark:text-white mb-3">Booking Confirmed!</h1>
                <p className="text-[#4B5A69] dark:text-slate-400 mb-2 text-lg">Your appointment has been successfully booked.</p>
                {bookedAppointment && (
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-lg border border-gray-100 dark:border-slate-700 text-left mb-8 space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    />
                    <div>
                      <p className="font-extrabold text-[#112429] dark:text-white">Dr. {selectedDoctor?.name}</p>
                      <p className="text-[#055153] text-sm font-semibold">{selectedDoctor?.specialization}</p>
                    </div>
                  </div>
                  <div className="h-[1px] bg-gray-100 dark:bg-slate-700" />
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-1">Date & Time</p>
                      <p className="font-bold text-[#112429] dark:text-white">{formatShortDate(selectedDateObj?.date)}</p>
                      <p className="text-slate-500 dark:text-slate-400">{selectedSlot?.startTime} – {selectedSlot?.endTime}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-1">Type</p>
                      <p className="font-bold text-[#112429] dark:text-white">{consultationType === 'Video' ? 'Video Consultation' : 'In-Person Visit'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-1">Status</p>
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-bold">⏳ Pending Confirmation</span>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase mb-1">Patient</p>
                      <p className="font-bold text-[#112429] dark:text-white">{patientName}</p>
                    </div>
                  </div>
                </div>
                ...
                {bookedAppointment?.preMedicationSteps?.length > 0 && (
                  <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-100 dark:border-teal-800 rounded-2xl p-5 text-left mb-6">
                    <p className="text-[10px] font-extrabold tracking-widest text-[#055153] uppercase mb-3 flex items-center gap-1.5"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg> AI Pre-Visit Steps</p>
                    <ul className="space-y-1.5">
                      {bookedAppointment.preMedicationSteps.map((step, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                          <span className="w-5 h-5 rounded-full bg-[#055153] text-white text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">{i+1}</span>
                          {step}
                        </li>
                      ))}
                    </ul>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => { setCurrentStep(1); setBookingSuccess(false); setBookedAppointment(null); }}
            ...
            <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
              ...
              >
                ...
                <div>
                  <div className="mb-8">
                    <h2 className="text-3xl font-extrabold text-[#112429] dark:text-white mb-2">Finalize Booking</h2>
                    <p className="text-[#4B5A69] dark:text-slate-400">Review your appointment details and complete payment.</p>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-[28px] p-8 shadow-sm border border-gray-100 dark:border-slate-700 mb-6">
                    ...
                    <div className="space-y-4 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-[#112429] dark:text-white">{patientName}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-500 font-medium">Phone</span>
                        <span className="font-bold text-[#112429] dark:text-white">{contactPhone}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        ...
                        }`}>{consultationType === 'Video' ? '📹 Video Consultation' : '🏥 In-Person Visit'}</span>
                      </div>
                      <div className="h-[1px] bg-gray-100 dark:bg-slate-700" />
                      <div>
                        ...
                        />
                        <p className="text-[10px] text-slate-400 mt-2 font-medium">AI will analyze these notes to provide pre-medication suggestions.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 rounded-[28px] p-8 shadow-sm border border-gray-100 dark:border-slate-700 mb-8">
                    <p className="text-[10px] font-extrabold tracking-widest text-[#055153] uppercase mb-5 flex items-center gap-2">
                      <FiCreditCard className="w-3.5 h-3.5" /> SECURE PAYMENT METHOD
                    </p>
                    <div className="flex flex-col md:flex-row gap-4">
                      <div 
                        className="flex-1 p-6 rounded-2xl border-2 border-[#055153] bg-[#F2FDFE] dark:bg-teal-900/10 shadow-md relative"
                      >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                             <h4 className="font-bold text-[#112429] dark:text-white text-lg">Stripe Checkout</h4>
                             <p className="text-sm text-slate-500 font-medium">Safe & Secure Credit Card</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <p className="text-xs text-slate-400 mt-6 text-center italic font-medium">Secure checkout powered by Stripe. You will be redirected to complete payment securely.</p>
                  </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      {isSubmitting ? (
                        ...
                        <><FiCreditCard className="w-5 h-5" /> Confirm Booking & Pay Rs. 2625</>
                      )}
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="bg-white dark:bg-slate-800 rounded-[32px] p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                    <p className="text-[10px] font-extrabold tracking-widest text-[#055153] uppercase mb-6">Appointment Summary</p>
                      ...
                      <div>
                        ...
                        {selectedDoctor?.rating?.average > 0 && (
                          <p className="text-amber-500 text-xs font-bold flex items-center gap-1 mt-0.5">
                            <FiStar className="w-3 h-3 fill-amber-500" /> {selectedDoctor.rating.average.toFixed(1)} ({selectedDoctor.rating.count}+ reviews)
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 text-sm mb-6">
                      <div className="flex gap-3 items-center">
                        ...
                        <div>
                          ...
                          <p className="font-bold text-[#112429] dark:text-white">{formatShortDate(selectedDateObj?.date)}</p>
                          <p className="text-slate-500 dark:text-slate-400 text-xs">{selectedSlot?.startTime} – {selectedSlot?.endTime}</p>
                        </div>
                      </div>
                      <div className="flex gap-3 items-center">
                        <div className="w-9 h-9 rounded-xl bg-teal-50 dark:bg-teal-900/30 flex items-center justify-center shrink-0">
                        ...
                        <div>
                          <p className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">{consultationType === 'Video' ? 'Session Type' : 'Clinic Location'}</p>
                          <p className="font-bold text-[#112429] dark:text-white">
                            {consultationType === 'Video' ? 'Virtual Telemedicine Room' : (selectedDoctor?.location || selectedDoctor?.hospital || 'Central Medical Plaza')}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-6 pt-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500 font-medium font-semibold">Consultation Fee</span>
                        <span className="font-bold text-[#112429] dark:text-white">Rs. {doctorFee.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500 font-medium font-semibold">Service Tax (5%)</span>
                        <span className="font-bold text-[#112429] dark:text-white">Rs. {serviceTax.toFixed(2)}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-[#055153] text-white rounded-2xl p-6 flex justify-between items-center shadow-lg shadow-teal-900/20 mb-6">
                       ...
                       <span className="font-extrabold text-3xl font-mono">Rs. {totalAmount}</span>
                    </div>
