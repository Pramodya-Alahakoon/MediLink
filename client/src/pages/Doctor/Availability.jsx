    ...
    return (
      <div className="w-full h-full flex items-center justify-center p-8 bg-[#F8FAFB] dark:bg-slate-950 transition-colors duration-300">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-[#112429] dark:text-white mb-2">Doctor Profile Not Found</h2>
          <p className="text-[#64748B] dark:text-slate-400 mb-4">{profileError}</p>
          <p className="text-sm text-[#64748B] dark:text-slate-400">Make sure you registered as a doctor. Contact support if the issue persists.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full h-full lg:flex-row flex flex-col gap-6 p-6 overflow-hidden bg-slate-50/50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-transparent rounded-2xl">
        ...
        <div className="mb-6 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <div>
            <h1 className="text-[28px] font-bold text-[#0D1C2E] dark:text-white mb-1 tracking-tight">Manage<br/>Availability</h1>
            <p className="text-[#4B5A69] dark:text-slate-400 text-[15px]">Configure your working<br/>hours and session blocks.</p>
          </div>
        ...
        <div className="flex-1 bg-white dark:bg-slate-900 rounded-2xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] dark:shadow-[0_2px_12px_-4px_rgba(0,0,0,0.8)] border border-slate-100 dark:border-slate-800 p-6 flex flex-col h-0">
          ...
          {loading ? (
            <div className="flex items-center justify-center h-full">
              <Loader2 className="animate-spin text-[#055153] dark:text-teal-400" size={32} />
            </div>
          ) : (
                ...
                {weekData.map((day, idx) => (
                  <div key={idx} className="text-center">
                    ...
                    <div className={`text-[20px] font-bold ${
                      idx === 3 ? 'text-[#055153] dark:text-teal-400' : day.isBlocked ? 'text-gray-400 dark:text-slate-500' : 'text-[#0D1C2E] dark:text-slate-100'
                    }`}>
                      {day.dayNumber}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-[80px_1fr_1fr_1fr_1fr_1fr_1fr_1fr] gap-2 h-full min-h-[600px] absolute inset-0">
                  ...
                  <div className="flex flex-col border-r border-slate-100/50 dark:border-slate-800/60 pr-4 mt-4">
                    {['08:00 AM', '09:00 AM', '10:00 AM', '11:00 AM', '12:00 PM', '01:00 PM', '02:00 PM', '03:00 PM'].map((time, i) => (
                      <div key={i} className="h-24 text-[11px] font-semibold text-slate-400 dark:text-slate-500 text-right whitespace-nowrap">
                        {time}
                      </div>
                    ))}
                  </div>
                  ...
                  {weekData.map((day, dayIdx) => (
                    <div key={dayIdx} className={`relative flex flex-col border-r border-slate-100/50 dark:border-slate-800/60 last:border-r-0 ${day.isBlocked ? 'bg-slate-50/50 dark:bg-slate-800/40' : ''}`}>
                      {day.isBlocked ? (
                        <div className="absolute inset-x-0 top-0 bottom-0 bg-slate-100/50 dark:bg-slate-900/60 flex flex-col items-center justify-center p-4">
                          <Ban size={24} className="text-slate-300 dark:text-slate-500 mb-2" />
                          <span className="text-[11px] font-bold text-slate-400 dark:text-slate-400 tracking-wider">BLOCKED</span>
                        </div>
                      ) : (
                        <div className="h-full relative w-full pt-4">
                           {day.slots.map((slot, slotIdx) => {
                             ...
                             if (slot.status === 'available') {
                               bgStyles = "bg-[#ECFDF5] border-l-4 border-[#055153] text-[#055153] dark:bg-emerald-900/30 dark:border-emerald-500 dark:text-emerald-300";
                               content = <div className="text-[10px] font-bold tracking-wider mb-1 uppercase">AVAILABLE</div>;
                             } else if (slot.status === 'booked') {
                               bgStyles = "bg-[#EEF2FF] border-l-4 border-blue-600 text-blue-900 dark:bg-blue-900/40 dark:border-blue-500 dark:text-blue-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)]";
                               content = (
                               ...
                               bgStyles = "bg-[#F1F5F9] border-l-4 border-slate-400 text-slate-600 dark:bg-slate-800/60 dark:border-slate-500 dark:text-slate-300";
                             ...
                             return (
                              <button
                                ...
                                className={`text-left p-2.5 rounded-r-lg rounded-tl-sm rounded-bl-sm transition-all hover:bg-opacity-80 outline-none ${bgStyles} ${
                                  selectedSlot?._id === slot._id ? 'ring-2 ring-offset-2 ring-blue-400 dark:ring-blue-500 shadow-md z-10' : 'z-0'
                                }`}
                              >
                                {content}
                              </button>
              ...
              <div className="flex items-center justify-between pt-4 mt-2 bg-white dark:bg-slate-900">
                <div className="flex items-center gap-6 px-4 py-3 bg-slate-50/80 dark:bg-slate-800/70 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-bold text-slate-600 dark:text-slate-300">Available</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.4)]" />
                    <span className="text-[12px] font-bold text-slate-600 dark:text-slate-300">Booked</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-slate-400" />
                    <span className="text-[12px] font-bold text-slate-600 dark:text-slate-300">Blocked</span>
                  </div>
                </div>
                ...
                {selectedSlot && (
                  <div className="flex items-center gap-3 animate-fade-in pr-4">
                    ...
                    {selectedSlot.status === 'blocked' ? (
                      <button onClick={handleUnblockSlot} className="px-4 py-2 text-[12px] font-bold text-emerald-700 dark:text-emerald-200 bg-emerald-100 dark:bg-emerald-900/40 rounded-lg hover:bg-emerald-200 dark:hover:bg-emerald-900/60 transition-colors">
                        Unblock Slot
                      </button>
                    ) : selectedSlot.status === 'available' ? (
                      <button onClick={handleBlockSlot} className="px-4 py-2 text-[12px] font-bold text-slate-700 dark:text-slate-200 bg-slate-200 dark:bg-slate-700 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
                        Block Slot
                      </button>
                    ) : null}
                    <button onClick={() => setSelectedSlot(null)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
                      <X size={18} />
                    </button>
                  </div>
                )}
      ...
      <div className="w-full lg:w-[360px] flex-shrink-0 flex flex-col gap-5 overflow-y-auto px-1 pb-4 custom-scrollbar">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[20px] shadow-[0_2px_16px_-4px_rgba(0,0,0,0.06)] dark:shadow-[0_2px_14px_-4px_rgba(0,0,0,0.8)] border border-slate-100/60 dark:border-slate-800 p-6 pt-5">
          <div className="flex items-center gap-3 mb-6">
            <Settings2 size={20} className="text-[#055153] dark:text-teal-400" />
            <h2 className="font-bold text-[17px] text-[#0D1C2E] dark:text-white">Appointment Settings</h2>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <button 
                  ...
                  className={`w-11 h-6 rounded-full transition-colors relative shadow-inner ${
                    tempSettings.isBufferTimeEnabled ? 'bg-[#055153]' : 'bg-slate-300 dark:bg-slate-700'
                  }`}
                >
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                <CalendarIcon size={18} className="text-[#055153] dark:text-teal-400" />
                <span className="font-bold text-[15px] text-[#0D1C2E] dark:text-white">Block Days</span>
              </div>
              
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase mb-1.5 block tracking-widest">Start Date</label>
                  <input 
                    type="date"
                ...
                <div>
                  <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase mb-1.5 block tracking-widest">End Date</label>
                  <input 
                    type="date"
