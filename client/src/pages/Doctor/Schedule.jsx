...
const Schedule = () => {
    ...
    try {
      const fetchCount = async (status) => {
        const { data } = await customFetch.get(
          `/api/doctors/${doctorId}/appointments?status=${status}&limit=1`,
        );
        return data.total || 0;
      };
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const { data } = await customFetch.get(
        `/api/doctors/${doctorId}/appointments?status=${mappedStatus}&page=${page}&limit=10`,
      );
  ...
  const handleAction = async (action, appointmentId, extraData = {}) => {
    try {
      ...
      toast.error(
        error?.response?.data?.message || `Failed to ${action} appointment`,
      );
    }
  };
  ...
  const getFilteredAppointments = () => {
    ...
    return appointments.filter((apt) => {
      const patientMatch =
        ...
        (apt.contactPhone || "").includes(q);

      const dateMatch =
        !todayOnly ||
        (apt.appointmentDate && apt.appointmentDate.slice(0, 10) === todayIso);

      const specialization = apt.specialization || "General";
      const typeMatch =
        typeFilter === "All Types" || specialization === typeFilter;

      return patientMatch && dateMatch && typeMatch;
    });
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#F8FAFB] dark:bg-slate-950 flex flex-col p-6 lg:p-8 transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          <h1 className="text-[26px] xl:text-[30px] font-bold text-[#0D1C2E] dark:text-white mb-1 tracking-tight">
            Appointment Management
          </h1>
          <p className="text-[#64748B] dark:text-slate-400 text-[15px]">
      ...
      <div className="flex items-center gap-8 border-b border-slate-200 dark:border-slate-800 mb-6 overflow-x-auto custom-scrollbar">
        {["Upcoming", "Pending", "Completed", "Cancelled"].map((tab) => {
          let label = tab;
          if (tab === "Upcoming" && counts.Upcoming > 0)
            label = `Upcoming (${counts.Upcoming})`;
          if (tab === "Pending" && counts.Pending > 0)
            label = `Pending Requests (${counts.Pending})`;
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 border rounded-xl shadow-sm transition-colors ${
              todayOnly
                ? "bg-[#055153] text-white border-[#055153] dark:bg-teal-600 dark:border-teal-500"
                : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-[#0D1C2E] dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800"
            }`}
          >
            <Calendar
              size={18}
              className={
                todayOnly ? "text-white" : "text-[#0D1C2E] dark:text-slate-100"
              }
            />
            <span className="font-semibold text-[14px]">
              {todayOnly ? "Today: ON" : "Today"}
            </span>
          </button>

          <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            <Filter size={18} className="text-[#0D1C2E] dark:text-slate-100" />
            <select
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
              className="bg-transparent text-[#0D1C2E] dark:text-slate-100 font-semibold text-[14px] pr-1 outline-none"
            >
              {availableTypes.map((type) => (
                <option key={type} value={type} className="text-[#0D1C2E]">
                  {type}
                </option>
              ))}
            </select>
      ...
      <div className="flex-1 flex flex-col gap-4 relative">
        {loading ? (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center border border-slate-200 dark:border-slate-800 shadow-sm flex-1 flex flex-col items-center justify-center">
            ...
            <h3 className="text-[18px] font-bold text-slate-700 dark:text-slate-100 mb-1">
              No appointments found
            </h3>
            <p className="text-slate-500 dark:text-slate-400">
              There are no {activeTab.toLowerCase()} appointments right now.
            </p>
          </div>
        ) : (
          filteredAppointments.map((apt) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 flex-1">
                ...
                <div>
                  <h3 className="text-[17px] font-bold text-[#0D1C2E] dark:text-white mb-1">
              ...
              <div className="flex items-center gap-3 w-48 shrink-0">
                <div className="w-10 h-10 rounded-full bg-[#F1F5F9] dark:bg-slate-800 flex items-center justify-center shrink-0">
                  <Clock
                    size={18}
                    className="text-[#0D1C2E] dark:text-slate-100"
                  />
                </div>
                <div>
              ...
              <div className="flex flex-col gap-2 items-start lg:items-center w-32 shrink-0">
                <div
                  className={`px-3 py-1 text-[11px] font-bold rounded-full flex items-center gap-1 uppercase tracking-wider
                  ${apt.specialization === "General Medicine" ? "bg-[#EEF2FF] text-[#3B82F6]" : "bg-[#ECFEFF] text-[#0891B2] dark:bg-sky-900/40 dark:text-sky-300"}`}
                >
                  {apt.specialization === "General Medicine" ? (
                    <User size={12} />
                  ) : (
                    <Video size={12} />
                  )}
                ...
                {apt.status === "Completed" && (
                  <>
                    <button
                      ...
                      className="px-5 py-2.5 bg-[#055153] hover:bg-[#044042] dark:bg-teal-600 dark:hover:bg-teal-500 text-white font-bold text-[13px] rounded-xl transition-colors shadow-sm flex items-center gap-2"
      ...
      {!loading && filteredAppointments.length > 0 && (
        <div className="flex items-center justify-between mt-8 pt-4">
          <span className="text-[14px] font-medium text-slate-500 dark:text-slate-400">
            Showing {(page - 1) * 10 + 1}-
            {Math.min((page - 1) * 10 + filteredAppointments.length, page * 10)}{" "}
            of {filteredAppointments.length} filtered appointments
          </span>
          <div className="flex items-center gap-2">
            <button
              disabled={page === 1}
              onClick={() => setPage((p) => Math.max(1, p - 1))}
              className="w-10 h-10 rounded-full flex items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-[#0D1C2E] dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              <ChevronLeft size={18} />
            </button>
            <div className="w-10 h-10 rounded-full flex items-center justify-center bg-[#055153] dark:bg-teal-600 text-white font-bold text-[14px] shadow-sm">
              {page}
            </div>
            <button
