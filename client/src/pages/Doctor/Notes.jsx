...
const NOTE_COLORS = [
  {
    card: "bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-700/40",
    accent: "text-amber-600",
  },
  {
    card: "bg-sky-50 border-sky-200 dark:bg-sky-900/20 dark:border-sky-700/40",
    accent: "text-sky-600",
  },
  {
    card: "bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-700/40",
    accent: "text-emerald-600",
  },
  {
    card: "bg-rose-50 border-rose-200 dark:bg-rose-900/20 dark:border-rose-700/40",
    accent: "text-rose-600",
  },
  {
    card: "bg-violet-50 border-violet-200 dark:bg-violet-900/20 dark:border-violet-700/40",
    accent: "text-violet-600",
  },
  {
    card: "bg-teal-50 border-teal-200 dark:bg-teal-900/20 dark:border-teal-700/40",
    ...
    (async () => {
      try {
        ...
        const { data } = await customFetch.get(
          `/api/doctors/${resolvedDoctorId}/notes`,
        );
    ...
    async (id, updates) => {
      ...
      try {
        await customFetch.put(
          `/api/doctors/${resolvedDoctorId}/notes/${id}`,
          updates,
        );
        setDirty((prev) => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 max-w-xl">
            Quick sticky notes for clinical reminders. Changes auto-save after
            you stop typing, or click Save.
          </p>
        </div>
      ...
      {!loading && notes.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-slate-400 dark:text-slate-500 text-sm text-center max-w-xs">
            Click "Add Note" to create your first sticky note for quick
            reminders.
          </p>
        </div>
      )}
              ...
              >
                ...
                <div className="px-4 pb-2 flex-1">
                  <textarea
                    ...
                    value={note.content || ""}
                    onChange={(e) =>
                      handleChange(note._id, "content", e.target.value)
                    }
